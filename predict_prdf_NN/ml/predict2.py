...
def predict(atom_category, input_list, broad=0.05, erange=10):
            ...
            if ( j == 'DOS_%3.2f_%2.2i' % (broad, erange) ) and ( int(i.split('_')[2]) == n_dop ):
                ...
                for atom1 in at1:
                    ...
                    for atom2 in at2:
                        ...
                        distance = abs(c2-c1)
                        area = np.pi * (distance**2-(distance-dr)**2)
                        #prdf += gaussian( np.linspace(0, r_cut, N_grid), distance, sigma ) * (1/distance**2)
                        prdf += gaussian( np.linspace(0, r_cut, N_grid), distance, sigma ) / area
    ...
    hidden_layer_sizes = (N_grid, 3*N_grid, N_grid)
    ...
    for atom1 in at_input:
        ...
        for atom2 in at2_input:
            ...
            prdf_input += gaussian( np.linspace(0, r_cut, N_grid), distance, sigma ) / area
